...
class AppLoggerTest {
    ...
    fun testLogLimitEdgeCase() {
        ...
        // The logs are added to the beginning of the list, so the first element in the list is the most recent log
        // Let's verify that the oldest log (Message 1) was dropped, and the newest log (Message 501) is present
        assertTrue("Most recent log should be present", logs.first().contains("Message $overLimit"))
        assertTrue("Oldest log should be dropped, second oldest should be present at the end", logs.last().contains("Message 2"))
    }
